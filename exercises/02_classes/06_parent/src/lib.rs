...
impl AccountWithHistory {
    ...
    fn new(balance: f64) -> PyClassInitializer<Self> {
        ...
        let account_history = AccountWithHistory {
            history: Vec::new(),
        };
        PyClassInitializer::from(account).add_subclass(account_history)
    }
    ...
    fn set_balance(mut self_: PyRefMut<'_, Self>, balance: f64) {
        let curr_balance = self_.as_super().balance;
        self_.history.push(curr_balance);
