...
impl ShoppingOrder {
    ...
    fn new(name: String, price: Bound<'_, PyInt>, quantity: Bound<'_, PyInt>) -> PyResult<Self> {
        ...
        let price: u64 = price
            .extract()
            .map_err(|_| PyValueError::new_err("price should be non-negative"))?;
        if price == 0 {
            return Err(PyValueError::new_err("price should be non-zero"));
        }
        let quantity = quantity
            .extract()
            .map_err(|_| PyValueError::new_err("quantity should be non-negative"))?;
        if quantity == 0 {
            return Err(PyValueError::new_err("quantity should be non-zero"));
        }
        Ok(ShoppingOrder {
            name,
            price,
